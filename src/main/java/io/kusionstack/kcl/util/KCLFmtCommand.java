...
/**
 * @author amyxia
 * @version KCLFmtCommand: KCLFmtCommand.java, v 0.1 2020年12月03日 2:53 下午 amyxia
 *          Exp $
 */
public class KCLFmtCommand {
    ...
    public static boolean execute(@NotNull VirtualFile virtualFile, boolean isDirectory) {
        ...
        if (!result.isSuccess()) {
            LOGGER.error(
                    String.format("kcl fmt %s exec failed, err msg: %s", virtualFile.getPath(), result.getStderr()));
        }
